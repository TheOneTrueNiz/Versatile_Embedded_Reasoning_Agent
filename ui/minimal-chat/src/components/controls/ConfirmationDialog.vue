...
$warning-color: var(--vera-danger);
$warning-dark: var(--vera-danger);
$background-color: var(--vera-glass-strong);
$overlay-color: rgba(var(--vera-shadow-rgb), 0.75);
...
.confirmation-dialog {
  ...
  width: 100%;
  max-width: 450px;
  box-shadow: 0 8px 30px rgba(var(--vera-shadow-rgb), 0.5);
  overflow: hidden;
  display: flex;
  flex-direction: column;
  align-items: center;
  padding: 30px;
  border: 1px solid rgba(var(--vera-accent-rgb), 0.3);
  ...
  .dialog-content {
    ...
    p {
      ...
      line-height: 1.5;
      margin: 0 0 25px 0;
      color: color-mix(in srgb, var(--vera-text) 80%, transparent);
    }
  }
  ...
  .dialog-actions {
    ...
    .cancel-btn {
      ...
      &:hover {
        background-color: rgba(var(--vera-contrast-rgb), 0.2);
      }
    }
