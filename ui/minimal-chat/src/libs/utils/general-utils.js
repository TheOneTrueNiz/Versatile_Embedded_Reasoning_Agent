...
// Toast stack management - simple counter approach
let activeToastCount = 0;
const TOAST_GAP = 10; // Gap between stacked toasts
const TOAST_HEIGHT = 56; // Approximate height of a toast (padding + content)

export function showToast(message, options = {}) {
  const duration = typeof options.duration === 'number' ? options.duration : 2000;
  const yOffset = activeToastCount * (TOAST_HEIGHT + TOAST_GAP);
  activeToastCount++;
  ...
  Toastify({
    ...
    style: {
      ...
      color: 'var(--vera-text)',
      boxShadow: '0 4px 12px rgba(var(--vera-shadow-rgb), 0.4), 0 2px 4px rgba(var(--vera-shadow-rgb), 0.3)',
      border: '1px solid var(--vera-accent-30)',
      borderRadius: '12px',
      padding: '14px 20px',
      fontFamily: 'var(--vera-font-sans)',
      fontSize: '16px',
      lineHeight: '1.4',
      maxWidth: '80vw',
      overflow: 'hidden',
    ...
    onClick: function () { },
    callback: function () {
      // Decrement counter when toast is dismissed
 ...
 * Resizes large images and applies JPEG compression.
 ...
 * @param {number} options.maxSizeKB - Target max size in KB (default 1024)
    ...
    reader.onload = (e) => {
      img.onload = () => {
        try {
          ...
          while (dataUrl.length > maxSizeKB * 1024 * 1.37 && currentQuality > 0.3) {
            currentQuality -= 0.1;
            dataUrl = canvas.toDataURL('image/jpeg', currentQuality);
          }

          console.log(`Image compressed: ${file.size} bytes -> ~${Math.round(dataUrl.length / 1.37)} bytes (quality: ${currentQuality.toFixed(1)})`);
          resolve(dataUrl);
        } catch (err) {
          reject(err);
        }
