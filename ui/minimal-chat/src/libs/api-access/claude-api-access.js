...
function getAnthropicMessagesUrl() {
  const viteBase = import.meta?.env?.VITE_ANTHROPIC_BASE_URL;
  const processBase = typeof process !== 'undefined' && process?.env ? process.env.ANTHROPIC_BASE_URL : undefined;
  const rawBase = viteBase || processBase || '/api/anthropic';
