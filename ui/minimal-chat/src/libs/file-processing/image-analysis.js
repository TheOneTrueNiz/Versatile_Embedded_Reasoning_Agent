...
import { compressImageForUpload } from '../utils/general-utils.js';

// Helper functions - use compression to avoid 413 errors
...
export async function analyzeImage(file, fileType, messages2, model, localModelName, localModelEndpoint, promptText = '') {
    ...
    const prompt = promptText.trim() || 'Describe this image.';
    const formattedMessages = formatMessagesForVisionAPI(messages2, base64Image, prompt);

    // Handle different model types
    if (model.includes('gpt') || selectedModel.value.includes('open-ai-format')) {
        const visionMessage = createGPTVisionMessage(base64Image, prompt);
