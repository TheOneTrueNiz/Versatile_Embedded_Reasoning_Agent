...
class IndexManager:
    ...
    def _get_gpu_id(self) -> int:
        try:
            return int(os.getenv("MEMVID_GPU_ID", "0"))
        except ValueError:
            return 0

    def _gpu_available(self) -> bool:
        if os.getenv("MEMVID_USE_GPU", "1") != "1":
            return False
    ...
    def _init_embedding_model(self):
        ...
        try:
            return SentenceTransformer(self.config["embedding"]["model"], device=device)
        except TypeError:
    ...
    def _create_index(self) -> faiss.Index:
        ...
        if self.use_gpu:
            try:
                ...
                index = faiss.index_cpu_to_gpu(self._gpu_resources, self.gpu_id, index)
                logger.info(f"FAISS GPU enabled (gpu_id={self.gpu_id})")
            except Exception as e:
                self.use_gpu = False
                logger.warning(f"GPU index init failed, falling back to CPU: {e}")
    ...
    def load(self, path: str):
        ...
        if self.use_gpu and hasattr(faiss, "index_cpu_to_gpu"):
            try:
                ...
                self.use_gpu = False
                logger.warning(f"GPU index load failed, using CPU: {e}")
        self.index = index
